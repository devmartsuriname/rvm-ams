...
const LogoLgContent = ({ textColor }: { textColor: string }) => (
  <span className="logo-lg" style={{ display: 'inline-flex', alignItems: 'center', gap: 10, height: 36 }}>
    <IconifyIcon icon={ICON} style={{ color: ICON_COLOR, fontSize: 30, flexShrink: 0 }} />
    <span style={{ display: 'inline-flex', flexDirection: 'column', justifyContent: 'center', lineHeight: 1 }}>
      <span style={{ fontFamily: 'Play, sans-serif', fontWeight: 700, fontSize: 20, color: textColor, whiteSpace: 'nowrap' }}>RVM Flow</span>
      <span style={{ fontFamily: 'Play, sans-serif', fontWeight: 400, fontSize: 12, color: MUTED, whiteSpace: 'nowrap' }}>Management System</span>
    </span>
  </span>
)
...
const LogoSmContent = () => (
  <span className="logo-sm" style={{ display: 'none', width: 24, height: 24, alignItems: 'center', justifyContent: 'center' }}>
    <IconifyIcon icon={ICON} style={{ color: ICON_COLOR, fontSize: 26 }} />
  </span>
)
